...
export class AlunoFactory extends Factory {
    ...
    public static async upd(model: Aluno, notify?: boolean) {
        try {
            let result = await this.post('/api/aluno/upd', model) as Aluno;
    ...
    public static async del(id: string, notify?: boolean) {
        try {
            let result = await this.delete('/api/alunodel', { params: { id: id } });
