...
import { ProfessorModel } from '../../../../../ezs-common/src/model/server/professor.model';
...
@Component({
    template: require('./page-usuario-professor.html')
})
export class PageUsuarioProfessorComponent extends Vue {
    ...
    model: ProfessorModel = new ProfessorModel();
    ...
    async mounted() {
        try {
            ...
            this.model = await Factory.UsuarioFactory.detailProfessor(this.$route.params.id);
        }
        catch (e) {
    ...
    async save() {
        try {
            AppBroadcastEventBus.$emit(AppBroadcastEvent.EXIBIR_LOADER);
            await Factory.UsuarioFactory.updateProfessor(this.$route.params.id, this.model);
