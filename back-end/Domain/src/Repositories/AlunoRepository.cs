...
namespace Domain.Repositories {
    public class AlunoRepository : IRepository<Aluno> {
        ...
        public Aluno Get (string ID) => this.db.Alunos.Include (i => i.UsuarioInfo).SingleOrDefault (x => x.ID == ID);
        public List<Aluno> GetAll (bool? ativo) => this.db.Alunos.Include (i => i.UsuarioInfo).Where (x => x.Ativo == (ativo.HasValue ? ativo.Value : false)).ToList ();
